...
export async function POST(request: NextRequest) {
	try {
		console.log("🚀 API route: submit-testimonial called");
		...
		console.log("🚀 Form data:", { name, role, company, quote, email, avatar: avatar?.name || "no file" });
		...
		if (avatar && avatar.size > 0) {
			...
			if (!validTypes.includes(avatar.type)) {
				return NextResponse.json(
					{ success: false, error: "Please upload a valid image file (JPG, PNG, or WebP)" },
					{ status: 400 }
				);
			}
		...
		return NextResponse.json(
			{ 
				success: false, 
				error: error.message || "Failed to submit testimonial. Please try again." 
			},
			{ status: 500 }
		);
	}
...
async function sendNotificationEmail(testimonial: any) {
	try {
		...
		console.log('📧 TO:', process.env.ADMIN_EMAIL || 'admin@webstudestudios.com')
		...
		const companyMatch = emailHtml.match(/<strong>Company:<\/strong> (.*?)<\/p>/)
		const quoteMatch = emailHtml.match(/<blockquote[^>]*>.*?"(.*?)".*?<\/blockquote>/)
		...
		console.log('-'.repeat(40))
		console.log('🔗 ADMIN URL: Login to your website to approve this testimonial')
		console.log('='.repeat(80) + '\n')
